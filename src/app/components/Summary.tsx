...
import { AlertTriangle, TrendingUp, TrendingDown, Target, Shield, Download } from "lucide-react";
...
export function Summary({
  ...
  const generateReport = () => {
    ...
    const reportContent = `
================================================================================
                    GEOPOLITICAL RISK PORTFOLIO ANALYSIS
                              SUMMARY REPORT
================================================================================
...
Portfolio Risk Score: ${riskScore.toFixed(0)}/100
Risk Level: ${riskLevel.label}
Global Average Risk: ${averageGlobalRisk}
Total Assets: ${portfolio.length}
Countries Exposed: ${uniqueCountries.size}
Total Portfolio Value: $${((portfolio.reduce((sum, asset) => sum + asset.value, 0) / 1000000) * 10).toFixed(1)}M
...
${normalizedRiskFactors.map((factor) => 
  `${factor.name}: ${factor.normalizedScore.toFixed(0)}%`
).join("\n")}
...
${portfolioAnalysis.topRiskAssets.slice(0, 10).map((asset, idx) => {
  const contrib = portfolioAnalysis.assetContributions.find((a) => a.ticker === asset);
  return `${idx + 1}. ${asset}: Risk Score ${contrib?.riskScore.toFixed(2) || "N/A"}`;
}).join("\n")}
...
${portfolioAnalysis.topRiskCountries.slice(0, 10).map((country, idx) => {
  const risk = riskData[country] || 0;
  const exposure = portfolioAnalysis.countryExposures.find((e) => e.country === country);
  return `${idx + 1}. ${country}: Risk ${risk.toFixed(0)}\n   Assets: ${exposure?.contributingAssets.join(", ") || "N/A"}`;
}).join("\n\n")}
...
${portfolio.slice(0, 20).map((asset, idx) => 
  `${idx + 1}. ${asset.ticker} (${asset.name})\n   Sector: ${asset.sector}\n   Weight: ${(asset.weight * 100).toFixed(1)}%\n   Value: $${(asset.value / 1000000).toFixed(2)}M`
).join("\n\n")}
...
Political:    ${(weights.political * 100).toFixed(1)}%
Economic:     ${(weights.economic * 100).toFixed(1)}%
Conflict:     ${(weights.conflict * 100).toFixed(1)}%
Corruption:   ${(weights.corruption * 100).toFixed(1)}%
Terrorism:    ${(weights.terrorism * 100).toFixed(1)}%

================================================================================
COUNTRY EXPOSURES
================================================================================

${portfolioAnalysis.countryExposures.slice(0, 20).map((exposure, idx) => {
  const risk = riskData[exposure.country] || 0;
  return `${idx + 1}. ${exposure.country}\n   Risk Contribution: ${(exposure.riskContribution * 100).toFixed(1)}%\n   Country Risk: ${risk.toFixed(0)}\n   Assets: ${exposure.contributingAssets.join(", ")}`;
}).join("\n\n")}

================================================================================
Report generated by Geopolitical Risk Dashboard
Source: Global Geopolitical Snapshot (April 2026)
================================================================================
`;
    ...
    link.href = url;
    link.download = `Portfolio_Risk_Report_${new Date().toISOString().split("T")[0]}.txt`;
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 px-3 py-1 rounded text-xs font-semibold bg-blue-600 hover:bg-blue-700 text-white transition-colors"
